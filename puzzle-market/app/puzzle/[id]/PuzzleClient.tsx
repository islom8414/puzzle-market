...
import type { PuzzleDetail } from "@/lib/puzzle-detail";
...
export default function PuzzleClient({
  initialSlug,
  initialCatalogPuzzle,
}: {
  initialSlug: string;
  initialCatalogPuzzle: PuzzleDetail | null;
}) {
  const params =
    useParams();

  const slug =
    String(params.id || initialSlug);

  const [catalogPuzzle, setCatalogPuzzle] =
    useState<CatalogPuzzle | null>(
      initialCatalogPuzzle
    );

  const [catalogLoading, setCatalogLoading] =
    useState(!initialCatalogPuzzle);
    ...
    catalogPuzzle
      ? {
          ...
          description: [
            ...
            catalogPuzzle.active_listing_count
              ? `${catalogPuzzle.active_listing_count} fragment listings are currently available.`
              : "Available fragments appear in the marketplace when listed.",
            catalogPuzzle.lowest_price
              ? `Current listings start at $${catalogPuzzle.lowest_price}.`
              : "",
          ]
            .filter(Boolean)
            .join(" "),
